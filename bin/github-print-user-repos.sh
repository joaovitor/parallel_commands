...
github_print_user_repos::main(){
    ...
    user_verification=$(
        curl -H "Authorization: token $GITHUB_TOKEN" \
            ...
            -LI \
            -w "%{http_code}\n" \
            -s "https://api.github.com/users/${GITHUB_OWNER}"
    )

    if [[ ! "${user_verification}" -eq "200" ]]; then
        echo "invalid user [$GITHUB_OWNER]"
        github_print_user_repos::usage
    fi

    while [ ${page} -eq 1 ] || [ ${returned_repos} -ge ${per_page} ]; do
        curl -H "Authorization: token $GITHUB_TOKEN" -s "https://api.github.com/users/${GITHUB_OWNER}/repos?per_page=${per_page}&amp;page=${page}" -o "${temp_file}"
